...
The reference implementation supports the minimal v0.0.1 transaction: an absolute `gofer://` URI with explicit host and port, one selector, one Core response with an explicit terminator, and connection close. It uses plain TCP. It does not implement TLS, authentication, automatic traversal, recursive LINK following, service execution, MCP, ontology interpretation, persistence, or discovery registries. The reference client does not automatically follow LINK records and limits each complete Core response to 1 MiB (1,048,576 wire bytes), including record framing and the completion marker. This is a reference-client safety policy; Gopher-NG v0.0.1 defines no protocol response-size limit.
